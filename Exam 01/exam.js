let militaryUnit = {
  ...
  const Mission1 = militaryUnit => {
    let military = militaryUnit.commandStructure.executiveOfficer.chiefOfStaff;
    return `${military.rank} ${military.name} ${military.contact.phone} `
  }
console.log(Mission1(militaryUnit));


const Mission2 = militaryUnit => {
    const n = militaryUnit.personnel.length;
    return `The number of personnel is ${n}.`
}

console.log(Mission2(militaryUnit));

const Mission3 = (military,newDevelopment) =>{
        const current = {
            location: "Middle East",
            eventDate: "1941-06-01",
        ...
        military.current = newDevelopment;
...
const Mission4 = (weapon)=>{
    military.equipment.firearms
    .reduce((acc,cur)=> acc.includes(weapon)?acc.quantity++ : [...cur],[] )
